...
#endif
#include <iostream>
class Exercise2_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Exercise2_api_t(VExercise2* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_data_in_0)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_data_in_1)));
        ...
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_data_out_0)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_data_out_1)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Exercise2.reset"] = 0;
    }
#if VM_TRACE
...
static Exercise2_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VExercise2* top = new VExercise2;
    std::string vcdfile = "test_run_dir/lab8/Exercise2.vcd";
